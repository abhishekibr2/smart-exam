...
import { deleteFooterMenu, getFooterMenus, updateFooterOrderOfMenu } from '@/lib/adminApi';
import { FaEdit, FaTrash } from 'react-icons/fa';
import { RootState } from '@/redux/store';
import { useAppDispatch, useAppSelector } from '@/redux/hooks';
import { FooterMenu } from '@/lib/types';
import { setDataSource } from '@/redux/reducers/footerMenuReducer';

interface Props {
    onEdit: (data: FooterMenu) => void
    activeKey: string;
}
...
export default function HeaderData({ activeKey, onEdit }: Props) {
    ...
    const { dataSource, reload } = useAppSelector((state: RootState) => state.footerMenuReducer);
    ...
    const handleDelete = async (menuId: string) => {
        try {
            const res = await deleteFooterMenu({ menuId: menuId });
            if (res.status === true) {
    ...
    const fetchData = async () => {
        try {
            ...
            if (res.status === true) {
                const formattedData = (res.data as FooterMenu[])
    ...
    const onDragEnd = async ({ active, over }: DragEndEvent) => {
        if (active.id !== over?.id) {
            try {
                ...
                const response = await updateFooterOrderOfMenu(payload);
